...
def generate_target_data(input_path, target_path):
   ...
   #             inline = inline.rstrip().split("\t")
   #             print(inline)
   #             # Prepends 'chr' to chromosome value from CNV file if it does not have it
   #             chrom = str(inline[1]) if 'chr' in str(inline[1]) else 'chr'+str(inline[1])
   #             start = inline[2]
   #             end = inline[3]
   #             try:
   #                 if(float(inline[5]) <= -0.20):
   #                     CNV = "L"
   #                 elif(float(inline[5]) <= 0.20):
   #                     CNV = "N
   #                 else:
   #                     CNV = "G"
   #             except:
   #                 continue
    ...
    #            outline = str(chrom) + "\t" + str(start) + "\t" + str(end) + "\t" + CNV + "\n"
    #            outfile.write(outline)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Data preprocessing")
    parser.add_argument('--bam', type=str, help='location of the bam file')
    parser.add_argument('--ref', type=str, default='~/truman/EXCNV/data/hg19.fa', help='location of reference file')
